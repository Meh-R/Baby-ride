...
export async function createCartHasProduct(id: string, quantity: number) {
  let url = `${process.env.NEXT_PUBLIC_API_URL}product/createCartHasProduct/${id}`;
  let axiosConfig = {
    headers: {
      "content-type": "application/json;charset=utf-8",
      "Access-Control-Allow-Origin": "*",
...
export async function updateCartHasProduct(id: string, quantity: number) {
  let url = `${process.env.NEXT_PUBLIC_API_URL}product/updateCartHasProduct/${id}`;
